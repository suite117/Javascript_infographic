...
function initClouds(id) {
	...
	var zoomBehavior = d3.behavior.zoom().on("zoom", zoomIn);
	g.call(zoomBehavior);

	//g.on("contextmenu", zoomOut);

}

function zoomOut(d, id) {
	var oldscale = d.scale;
	d.scale = oldscale - 0.1;
	d.x = (d.x / oldscale) * d.scale;
	d.y = (d.y / oldscale) * d.scale;

	d3.select(this).attr("transform", transform);
	// disabilita menu contestuale del click pulsante destro
	d3.event.preventDefault();
}

function zoomIn(d, id) {
	...
	d.x = 0; //d.x + d.x * (d.scale - oldscale);
	d.y = 0;//d.y + d.y * (d.scale - oldscale);
