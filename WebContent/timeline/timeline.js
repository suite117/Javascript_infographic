...
Timeline.prototype.draw = function(data, selected) {
	...
	for ( var i = 0; i < data.length; i++) {
		...
		row[2] = '<div id="' + rowId + '">' + data[i][this.name] + '</div>';
		$("#" + rowId).data("id", data[i][this.idField]);

		if (data[i].image != undefined)
			row[2] += '<img src="' + data[i].image
	...
	function drawVisualization() {
		...
		$('#' + destinationDivId)
				.prepend(
						'Inizio tempo:<input type="text" id="'
								+ destinationDivId
								+ '-startDate" value="" size="10">Fine tempo:<input type="text" id="'
								+ destinationDivId
								+ '-endDate" value="" size="10" ><input type="button" id="'
								+ destinationDivId
								+ '-setRange" value="Set" ><input type="button" id="'
								+ destinationDivId
								+ '-setCurrentTime" value="Data corrente">');
		...
		// ondelete);
		google.visualization.events.addListener(timeline, 'rangechange',
				onrangechange);
		...
		$('#' + destinationDivId + '-setRange').on(
				...
				function(e) {
					...
					var newStartDate = new Date($(
							'#' + destinationDivId + '-startDate').val());
					var newEndDate = new Date($(
							'#' + destinationDivId + '-endDate').val());
	...
	var onselect = function(event) {
		...
		if (rowIndex != undefined) {
			...
			$("#" + destinationDivId)
					.trigger("mapClicked", [ id, !isSelected ]);
			// console.log("isSelected", isSelected);

		} else {
			// document.getElementById("info").innerHTML += "no item
			// selected<br>";
		}
	};
	...
	var onchange = function() {
		...
		// document.getElementById("info").innerHTML += "item " + row + "
		// changed<br>";
	};
