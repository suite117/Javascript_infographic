...
function initClouds(idDOM, dominio, optional) {
	...
	/*
	 * // rect's button container coordinates var x = 150; var y = 80; //
	 * add union button var button = g.append("g").attr("class",
	 * "button").attr("transform", "translate(0,180)");
	 * button.append("rect").attr("x", x - 10).attr("y", y -
	 * 37).attr("width", 200).attr("height", 50);
	 * button.append("text").attr("x", x).attr("y", y).text(function(d) {
	 * return "unione"; }); button.on("click", function(d, i) { // select
	 * the cloud under the selected cloud var set = svg.select("g." +
	 * STATE.UNDER);
	 * 
	 * var b = set.data()[0]; // add elements of overlying set to active set
	 * var union = d.elements.union(b.elements); var d2 = clone(d); d2.id =
	 * clouds.length; d2.x += 50; d2.y += 50; d2.elements = union;
	 * clouds.push(d2); // remove overlying set from data
	 * //clouds.remove(b.id); // remove svg and redraw
	 * divSVG.select("svg").remove(); draw();
	 * 
	 * }); // add intersection button y += 50; button =
	 * g.append("g").attr("class", "button").attr("transform",
	 * "translate(0,180)"); button.append("rect").attr("x", x -
	 * 10).attr("y", y - 37).attr("width", 200).attr("height", 50);
	 * button.append("text").attr("x", x).attr("y", y).text(function(d) {
	 * return "intersezione"; }); button.on("click", function(d, i) { //
	 * select the cloud under the selected cloud var set = svg.select("g." +
	 * STATE.UNDER);
	 * 
	 * var b = set.data()[0]; // add elements of overlying set to active set
	 * var intersect = d.elements.intersect(b.elements); // remove overlying
	 * set from data //clouds.remove(d.id); //clouds.remove(b.id);
	 * 
	 * var d2 = clone(d); d2.id = clouds.length; d2.x += 50; d2.y += 50;
	 * d2.elements = intersect; clouds.push(d2); // remove svg and redraw
	 * divSVG.select("svg").remove(); draw();
	 * 
	 * }); // add difference button y += 50; button =
	 * g.append("g").attr("class", "button").attr("transform",
	 * "translate(0,180)"); button.append("rect").attr("x", x -
	 * 10).attr("y", y - 37).attr("width", 200).attr("height", 50);
	 * button.append("text").attr("x", x).attr("y", y).text(function(d) {
	 * return "differenza"; }); button.on("click", function(d, i) { //
	 * select the cloud under the selected cloud var set = svg.select("g." +
	 * STATE.UNDER);
	 * 
	 * var b = set.data()[0]; // add elements of overlying set to active set
	 ...
	 * 
	 * var d2 = clone(d); d2.id = clouds.length; d2.x += 50; d2.y += 50;
	 * d2.elements = clone; clouds.push(d2); // remove svg and redraw
	 * divSVG.select("svg").remove(); draw();
	 * 
	 * }); // add event listener on drag event var dragBehavior =
	 * d3.behavior.drag().on("drag", drag); g.call(dragBehavior);
	 */
    }
	...
	if (d.state != STATE.DRAGGABLE && (d.y > height / 30)) {
	    ...
	    // var textContent =
	    // d3.select(cloud).select("text")[0][0].textContent;
	    // var pathContent =
	    // d3.select(cloud).select("path")[0][0].getAttribute("d");
	    var cloud = this;
...
function createDraggableCloud(divId, d, viewOption) {
	...
	drop : function(event, ui) {
	    $('div#pop-up').show().css('top', event.pageY).css('left', event.pageX + 20);

	    var dDroppable = $(event.target).data("d");
	    var dDraggable = $((ui.draggable).data("d"))[0];
	    $("#union").button().click(function() {
		var dOut = {};
		dOut.id = ++maxCloudIds;
		dOut.name = "Unione";
		dOut.domain = dDroppable.domain;
		dOut.x += 50;
		dOut.y += 50;
		dOut.elements = dDraggable.elements.union(dDroppable.elements);
		createDraggableCloud("draggable-container", dOut);
		...
		var dOut = {};
		dOut.id = ++maxCloudIds;
		dOut.name = "Intersezione";
		dOut.domain = dDroppable.domain;
		dOut.x += 50;
		dOut.y += 50;
		dOut.elements = dDraggable.elements.intersect(dDroppable.elements);
		createDraggableCloud("draggable-container", dOut);
	    });
		...
		var dOut = {};
		dOut.id = ++maxCloudIds;
		dOut.name = "Differenza";
		dOut.domain = dDroppable.domain;
		dOut.x += 50;
		dOut.y += 50;
		dOut.elements = dDraggable.elements.difference(dDroppable.elements);
		createDraggableCloud("draggable-container", dOut);
	    });
	},
	out : function() {
