function Table(destinationDivId, data, idField, optional) {
    ...
    var fieldsNames = data.getFields();
    var columnsNames = fieldsNames;
   
    if (optional != null) {
	imageField = optional.image ? optional.image : imageField;
	columnsNames = optional["columns-names"] ? optional["columns-names"] : columnsNames;
    }
	...
	this.columns.push({
	    "sTitle" : columnsNames[i] != null && columnsNames[i] != "" ? columnsNames[i] : " ",
	});
	if (fieldsNames[i] == idField)
	    this.indexIdColumn = i;
	else if (fieldsNames[i] == imageField)
...
Table.prototype.draw = function(data, selected) {
    ...
    this.table = $('#' + this.idMap).dataTable(
		...
		"fnRowCallback" : function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
		    ...
		    $('td:eq(' + indexImageColumn + ')', nRow).addClass("table-image").html(
			    '<img src="' + aData[indexImageColumn] + '" style="width:80px; margin:0; padding: 0;"></img>');
    ...
    /*
     * $('#' + this.idMap + " tbody tr").live('click', function() { var
     * tableCell = $('td', this); });
     */
